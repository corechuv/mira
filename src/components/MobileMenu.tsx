...
export default function MobileMenu({ open, onClose, topOffset = 0 }: { open: boolean; onClose: ()=>void; topOffset?: number }) {
  ...
  const styleBlock = { top: 'var(--header-h, 64px)', height: 'calc(100svh - var(--header-h, 64px))' } as React.CSSProperties

  return (
    <AnimatePresence>
      {open && (
        <>
          <motion.div className="fixed inset-x-0 bottom-0 z-[95] bg-black/40" style={styleBlock}
            onClick={onClose} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}/>
          <motion.aside className="fixed inset-x-0 bottom-0 z-[100] bg-white p-4 shadow-2xl overflow-y-auto"
            style={styleBlock} initial={{y:10, opacity:0}} animate={{y:0, opacity:1}} exit={{y:10, opacity:0}}>
            ...
            <div className="mt-1 grid grid-cols-1">
              {CATS.map(({id,label})=>(
                <button key={id} onClick={()=>goto(`/catalog?category=${id}`)} className="rounded-lg px-2 py-2 text-left text-[15px] hover:bg-slate-50">{label}</button>
              ))}
            </div>
